...
参考实现 `UIHandles/UIHandleLoader.cs`：框架内部按 `LayersModeEnum` 独立计算 delta，同一个 layer 的所有 UI 共享同一次采样，不同 layer 之间不会互相消耗帧时长。
...
- **同层多 UI**由 layer 入口只采样一次，然后把同一个 `frames` 传给该层所有 UI，避免同层多个调用者瓜分这一帧的时长；
- **跨层隔离**用 `LayersModeEnum` 独立记录上次采样时间，避免 `Vanilla_Mouse_Text`、`Vanilla_Resource_Bars`、`Mod_MenuLoad` 等不同 layer 互相影响；
- **极端帧率**（最小化 / 卡顿）钳制到 `[0.05, 5]`，避免单帧 60 倍速跳变。

热重载也要顾到。`ModSystem.Load` / `Unload` 里要清空 `_lastFrameTicksByMode`，否则热重载后第一次读取会吃掉一段超大时间差——这个细节在 `UIHandleLoader` 里有对应处理。

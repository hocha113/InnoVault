...
namespace InnoVault.UIHandles
{
    ...
    /// 例如 <c>Color.White * panelAlpha</c> 这样的写法可以直接传入 <see cref="AnimatedFloat"/> 实例
    /// </remarks>
    public struct AnimatedFloat
    {
        ...
        public AnimatedFloat(float initial = 0f, float speed = 0.12f, float snapEpsilon = 0.001f) {
            Current = initial;
            Target = initial;
            Speed = speed;
        ...
        /// <summary>
        /// 推进一次插值，<see cref="Current"/>会以<see cref="Speed"/>为系数靠近<see cref="Target"/>
        /// </summary>
        public void Update() {
            Current = MathHelper.Lerp(Current, Target, Speed);
        ...
        /// <summary>
        /// 隐式转换为<see cref="float"/>，使该结构体能在多数运算与绘制接口中无缝替代普通浮点数
        /// </summary>
        public static implicit operator float(AnimatedFloat a) => a.Current;
    }
}
